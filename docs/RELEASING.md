...
The native matrix in `.github/workflows/desktop-release.yml` produces all four OS/architecture combinations. It verifies the packaged Apple Silicon app and actually installs the Linux DEB and Windows NSIS packages before testing their installed applications. Intel macOS builds and unit/source checks run on Intel; a separate dependent job checks the original Intel ZIP through Rosetta on Apple Silicon. The hosted Intel VM could not create WebGL2, so this is explicitly translation compatibility, not physical Intel GPU verification. Linux and Windows CI use software WebGL; they do not demonstrate hardware GPU performance. The software test selects the supported minimum window, render distance and resolution scale to bound its CPU workload. Artifacts are downloadable from a successful [workflow run](https://github.com/HonzaCuhel/Lunacrust/actions/workflows/desktop-release.yml). The workflow does not create public GitHub Releases. Review the resulting binaries and manually publish a release only when ready.
...
On Linux hosts that restrict unprivileged user namespaces, an unpacked application may lack the required sandbox setup. The DEB installer includes a user-namespace check, conditional sandbox-helper permissions and an application-specific AppArmor profile; extracting an AppImage or running the unpacked directory does not run that installer. Keep the Chromium sandbox enabled when validating a target host. Native Ubuntu DEB installation and runtime results, along with the earlier unpacked VPS preflight limit, are documented in `PACKAGING_VERIFICATION.md`.

The runtime check validates the private asset origin, sandbox, context isolation, renderer APIs, runtime version, WebGL, bundled audio, save/load/delete and world worker startup. It launches with a temporary `LUNACRUST_USER_DATA` profile, requires graceful process exit and removes that profile before reporting success. A persistent file lock, close timeout or renderer exception fails the check. `npm run probe`, `npm run probe:survival`, `npm run probe:smoke` and `npm run probe:lan` also isolate their profiles. Never run a bespoke destructive test against a normal profile.
